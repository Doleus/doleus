...
def class_metadata_dataset() -> tuple[list[int], list[int], MoonwatcherClassification]:
    ...
    dataset.add_predictions_from_model_outputs(predictions)
...
class TestClassification:
    ...
    def test_slicing_by_metadata(self, moonwatcher_classification_dataset):
        ...
        slice_dataset = moonwatcher_classification_dataset.slice_by_metadata_value('class', 'A')
    ...
    def test_accuracy_checks(self, class_metadata_dataset):
        ...
        assert check_a_result["result"] == pytest.approx(0.90, abs=0.001), f"Class A accuracy check failed: {check_a_result}"
        assert check_b_result["result"] == pytest.approx(0.80, abs=0.001), f"Class B accuracy check failed: {check_b_result}"
        ...
        assert overall_result["result"] == pytest.approx(0.85, abs=0.001), f"Overall accuracy check failed: {overall_result}"
