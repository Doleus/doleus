...
def test_metadata_addition(moonwatcher_dataset):
    ...
    brightness_values = [
        dp.get_metadata("brightness") for dp in moonwatcher_dataset.datapoints
    ]
    assert all(
        isinstance(v, (float, int, np.float32, np.float64)) for v in brightness_values
    )
    ...
    retrieved_values = [
        dp.get_metadata("custom") for dp in moonwatcher_dataset.datapoints
    ]
...
def test_check_creation_and_execution(moonwatcher_dataset):
    ...
    suite = CheckSuite(name="test_suite", checks=[check])
...
def test_task_handling():
    ...
    binary_dataset = MoonwatcherClassification(
        name="binary_test", dataset=dataset, task=Task.BINARY.value, num_classes=2
    )
...
def test_invalid_inputs(dummy_dataset):
    ...
    with pytest.raises((ValueError, TypeError)):
        MoonwatcherClassification(
            name="test", task=Task.MULTICLASS.value, num_classes=10
        )
...
def test_slice_accuracy_mapping():
    ...
    mw_dataset = MoonwatcherClassification(
        name="test_accuracy", dataset=dataset, task=Task.MULTICLASS.value, num_classes=2
    )
