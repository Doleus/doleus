...
def create_pkl_exam_list(exams_folder_path, breast_level_annotations_file_path):
    ...
    if not Path(breast_level_annotations_file_path).exists():
        print(f"Couldn't find breast level annotations file path: {breast_level_annotations_file_path}")
        return

    # Create a pandas dataframe from the breast level annotations csv file
    try:
        breast_level_annotations = pd.read_csv(breast_level_annotations_file_path)
    except Exception as e:
...
class VinDrMammoDataset(Dataset):
    ...
    def __init__(self, exam_list_before_cropping_file, breast_level_annotations_file, exams_path,
        ...
        self.breast_level_annotations = pd.read_csv(breast_level_annotations_file)
        self.exams_path = exams_path  # The images folder in the vinDrMamoDataset contains one folder for each exam. Each exam folder contains 4 images. One image per view.
    ...
    def __getitem__(self, idx):
        ...
        for view in ['L-CC', 'R-CC', 'L-MLO', 'R-MLO']:
            if exam[view]:
                ...
                exam_id_row = annotations[annotations['image_id'] == img_id]["study_id"]
                assert len(
                ...
                img_path = Path(self.exams_path) / str(exam_id) / f"{img_id}.dicom"
                ...
                row = annotations.loc[annotations['image_id'] == img_id, ["breast_birads"]]
                # Make sure that there's only one row because the img id should be unique
                assert len(row) == 1, f"Expecting one row for image id {img_id}, because ids are unique"
...
create_pkl_exam_list(exams_folder_path=EXAMS_PATH, breast_level_annotations_file_path=BREAST_LEVEL_ANNOTATIONS_PATH)
...
for _ in range(dataset.__len__()):  # Using the dataset's length
    prediction = {
        'left_benign': random.uniform(0, 1),
        'right_benign': random.uniform(0, 1),
        'left_malignant': random.uniform(0, 1),
        'right_malignant': random.uniform(0, 1)
    }
...
def dataset_output_transform(datapoint):
    ...
    # Labels are a list of birad scores. I will map each birad score to an int
    label_to_int = {
        'BI-RADS 1': 0,
        'BI-RADS 2': 1,
        'BI-RADS 3': 2,
...
# Create moonwatcher dataset with predictions
...
mw_dataset = MoonwatcherDataset(
    ...
    # Map the numerical labels to the string labels
    label_to_name={0: 'BI-RADS 1', 1: 'BI-RADS 2', 2: 'BI-RADS 3', 3: 'BI-RADS 4', 4: 'BI-RADS 5'}
)

# # Assign the loaded predictions to the MoonwatcherDataset
mw_dataset.predictions = dummy_predictions

# TODO: Write a check for the dataset
